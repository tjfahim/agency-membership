<template>
  <MainLayout>
    ...
    <div v-if="showError" class="alert alert-danger mb-4">
    ...
    <div class="p-6 relative">
      ...
      <Button @click="createModal"
        class="block px-4 py-2 ml-5 !text-slate-200 !no-underline rounded bg-blue-600 hover:bg-gray-700 text-lg mb-4">
        Create Payment
      </Button>
      ...
      <!--Edit payment modal-->
      <EditPayment :popUpModalEdit="popUpModalEdit" @close="popUpModalEdit = false" :packages="packages" :users="users" :paymentData="paymentData"/>
       <EditPayment :popUpModalView="popUpModalView" @close="popUpModalView = false" :packages="packages" :users="users" :paymentData="paymentData"/>
      <!-- Data table -->
      <vue-good-table
        :columns="columns"
        :rows="filteredPayments"
        :search-options="{ enabled: true, placeholder: 'Search payments...' }"
        :pagination-options="{
          enabled: true,
          perPage: 10,
        ...
        <template #table-row="{ row, column }">
          <td v-if="column.field === 'actions'" class="text-center py-2">
            <div class="flex justify-center space-x-4">
              <Link :href="`/payment/${row.id}`" class="text-blue-600 hover:underline" title="View Payment">
                <i class="las la-eye text-green-500 text-xl"></i>
              </Link>
              <button @click="editModal(row.id)" class="text-yellow-600 hover:underline pr-3" title="Edit Payment">
                <i class="las la-edit text-xl"></i>
              </button>
              <button @click="deletePayment(row.id)" class="text-red-600 hover:underline" title="Delete Payment">
...
const editModal = async (id) => {
  try {
    ...
    popUpModalEdit.value = true; // show modal
  } catch (error) {
