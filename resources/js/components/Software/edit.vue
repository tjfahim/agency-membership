<template>
      <div v-if="popUpModalUpdate" class="fixed inset-0 z-50 flex justify-center items-center bg-black bg-opacity-50">

        <div class="absolute inset-0 "  @click="$emit('close')"></div>
        <form class="!bg-white  rounded-2xl shadow space-y-4 p-6 w-1/3 pt-15 relative" @click.stop
          @submit.prevent="updateSoftware">
          <!-- Name -->
          ...
          <div class="flex items-center space-x-2">
            <input type="checkbox" id="is_active" name="is_active" 
              class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" v-model="form.is_active" />
...
// Sync form when props.updateForm changes
...
const updateSoftware = () => {
  form.put(route('software.update', form.id), {
    preserveScroll: true,
    onSuccess: () => {
